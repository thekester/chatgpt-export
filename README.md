# ChatGPT Markdown Export

A lightweight browser extension for archiving ChatGPT conversations in durable, reusable formats: Markdown, HTML, and ZIP.

Repository: [github.com/thekester/chatgpt-export](https://github.com/thekester/chatgpt-export)

It works with Chrome, Chromium, Edge, and Firefox without an intermediary server. Exports use the data available from the current ChatGPT session and preserve media, files, sources, and metadata whenever available.

## Features

- Export the current conversation or the accessible conversation history.
- Live export progress with a percentage and stage label after an export is started.
- Downloadable privacy-conscious diagnostic log when an export fails or completes with partial errors; the latest log is kept locally so it can still be downloaded after reopening the popup.
- Markdown, HTML, both formats, or a native JEX file for Joplin.
- Images, attachments, and files generated by ChatGPT.
- Web sources, file citations, tables, lists, code blocks, and rich content.
- Optional export of branches and regenerations.
- Self-contained Markdown (`*.embedded.md`) with resources embedded as MIME/Base64 Data URIs.
- Optional raw JSON, manifest, detected capabilities, and SHA-256 checksums.
- Incremental history export to skip unchanged conversations.
- Automatic splitting of large histories into multiple ZIP archives.
- ZIP64 support for large archives and archives containing more than 65,535 files.
- Local HTML index with full-text search, dates, projects, and message counts.
- DOM fallback for exporting the displayed conversation when the internal API is unavailable.
- Clean conversation output: empty turns are omitted and per-message model/timestamp lines are not added to Markdown or HTML.
- No role labels in Markdown: the card marks the question and everything else is plain text, so `User`, `ChatGPT` and `Thinking` are never written out. When thinking summaries are kept, they read as ordinary text opened by their total duration line.
- User messages are displayed as a bold, left-aligned one-cell card sized to the text rather than to the page. With **Markdown with HTML elements** the card is a self-contained `<table>` block with a pale blue background and no borders, and the whole message goes inside the cell: sent images, attachments, pasted code and pasted tables included, in their original order and at any length. It is the only form that can guarantee the colour and the alignment, since a Markdown table's header cell inherits whatever the viewer's theme puts on `th`. With **Pure Markdown only** the card is a single-row table whose header cell is shaded by the viewer's own header style; multi-line questions stay in that one cell, with `<br>` line breaks, and images, attachments, code and tables follow it. A message with no sentence at all still gets one: an image or an attachment sent without a comment moves into the card, and a message holding nothing but pasted code or a pasted table is preceded by an empty card. Every user message is therefore a card in both flavors.

## Quick installation

Ready-to-load builds are generated in `dist/` by the build script:

```bash
python build.py
```

The script creates builds for Chrome, Chromium, Edge, and Firefox. Each build also includes a versioned ZIP archive such as `chatgpt-export-chrome-0.6.14.zip`.

### Chrome, Chromium, and Edge

1. Open `chrome://extensions` or `edge://extensions`.
2. Enable **Developer mode**.
3. Click **Load unpacked**.
4. Select the matching folder in `dist/`.

### Firefox

1. Open `about:debugging#/runtime/this-firefox`.
2. Click **Load Temporary Add-on**.
3. Select `dist/firefox/manifest.json` or the Firefox archive.

Firefox uses `background.scripts`, while Chromium-based browsers use `background.service_worker`. `build.py` adapts the manifest automatically for each browser.

## Usage

1. Open `chatgpt.com` and sign in.
2. Open the extension from the browser toolbar.
3. Choose the format and export options.
4. Click **Export this conversation** or **Export all history**.

Settings are stored locally in the popup. History exports may produce multiple ZIP files when the configured maximum size is reached; the default is 1 GiB per archive.

### Main options

| Option | Effect |
| --- | --- |
| Markdown / HTML / Both / JEX | Selects the output format. **Both** means Markdown and HTML. **JEX** creates a native Joplin import file for the current conversation; **Export all history** creates one `.jex` per conversation and groups them into size-limited ZIP archive(s). |
| Save images in a separate directory | Downloads images into an `images/` directory inside the export archive and keeps Markdown links to them. |
| Save attachments and generated files in a separate directory | Preserves documents and generated files in a `files/` directory inside the export archive and keeps Markdown links to them. |
| Markdown flavor | Choose **Markdown with HTML elements** or **Pure Markdown only**. It controls whether the Markdown file is allowed to contain raw HTML tags, and changes how a user question is rendered (see below). |
| Markdown images and files | Choose **Keep images and files as links** or **Embed images and files (Base64)**. Normal web links and source citations remain links. Embedding creates an `.embedded.md` file and makes it much larger. |
| Include thinking summaries | Off by default. When enabled, visible ChatGPT thinking summaries are grouped into one section opened by their total duration. In Markdown they read as ordinary text, with no role label; the HTML export keeps a `Thinking` heading. Hidden chain-of-thought is never exported. |
| Raw JSON data | Adds the source conversation JSON. |
| All branches | Includes available regenerations and branches. |
| Incremental export | Skips unchanged conversations from previous exports. |
| SHA-256 checksums | Adds file hashes to the manifest. |

When **Embed media in Markdown (Base64)** is selected as the only save option, the current conversation is downloaded directly as a single `.embedded.md` file, without a ZIP archive. A ZIP is created when another output or save option is enabled. **Keep media as links** leaves remote links unchanged, while **Save images** and **Save attachments and generated files** download local copies and link to them from the Markdown. **Allow external media** only grants permission to download remote resources; it does not enable Base64 embedding.

The standard self-contained format keeps the export as plain Markdown with embedded Base64 resources. The **JEX** format creates a native `.jex` archive with the metadata fields supported by Joplin, including title, creation/update dates, source, author, and tags. Images and files are stored as Joplin resources referenced from the note, instead of putting large Data URIs in the note. The note uses theme-neutral Markdown and follows Joplin’s active light or dark theme.

Select **JEX** when importing into Joplin. **Export this conversation** creates one `.jex`; **Export all history** creates one `.jex` per accessible conversation and packages those files into one or more ZIP archives. Extract the ZIP archive(s), then select **File → Import → JEX** in Joplin and import the extracted files. Each JEX contains its note and any downloaded images or files. The standard `.embedded.md` format is intended for readers that handle large Base64 Data URIs reliably, not for large Joplin notes.

JEX imports a note, not a standalone file outside Joplin’s notebook structure. Joplin places a note without a specified parent notebook into a default notebook, usually named `Imported`. This is expected Joplin behavior. You can move the note to any existing notebook after import.

Access to all websites is optional. It is requested only when the extension needs to retrieve images or files hosted outside ChatGPT. Without this permission, the main export still works and preserves remote links when necessary.

## Export structure

Depending on the selected options, an archive may contain:

```text
export.zip
├── index.html
├── manifest.json
├── _capabilities.json
├── _erreurs.txt
├── _diagnostic.log        # only when errors/partial failures occur
├── conversations/
│   └── 2026-09-20_conversation-title/
│       ├── conversation-folder-name.md
│       ├── conversation-folder-name.html
│       ├── conversation-folder-name.embedded.md
│       ├── metadata.json
│       ├── raw.json
│       ├── images/
│       └── files/
└── ...
```

Files are included only when relevant to the selected options or to the data actually available.

## Compatibility and fallbacks

ChatGPT’s internal endpoints are not public APIs and may vary by account, workspace, and deployment. The extension tests each capability independently: a `403` or `404` is recorded in `_capabilities.json`, then the export continues with the available data.

For the current conversation, if the internal JSON endpoint is unavailable, the extension rebuilds an export from the messages visible on the page. This fallback is less complete but still recovers the displayed content.

Files generated with `sandbox:` use multiple download endpoints. Standard, archived, Projects, and shared conversations are supported when the account exposes the corresponding endpoints.

## Development

The project requires neither Node.js nor a JavaScript compilation step. The Python build script assembles the sources, adapts the manifest, and creates ZIP archives.

```text
src/
├── background.js   # Downloads requiring host permissions
├── content.js      # ChatGPT access and export orchestration
├── render.js       # Markdown, HTML, and index rendering
└── zip.js          # Browser-side ZIP/ZIP64 generation
popup/
├── popup.html      # Configuration interface
└── popup.js
lib/
└── marked.umd.js   # Bundled Markdown renderer
```

Rebuild all distributions with:

```bash
python build.py
```

The `dist/` directory is recreated on every run.

## Privacy and limitations

- Exports are generated locally in the browser.
- The extension does not send data to a third-party service.
- Requests to ChatGPT use the session already open in the browser.
- External image requests are made only when the corresponding optional permission is granted.
- ChatGPT’s internal APIs may change without notice.
- The DOM fallback exports only the conversation currently displayed; it cannot replace API access for browsing the full history.
- Self-contained Markdown increases binary resource size by about 33% because of Base64 encoding. Some Markdown viewers may block `data:` URLs.

Private reasoning and hidden internal tool calls are not exported: only content displayed or exposed as conversation data is preserved.

## Version

Current version: **0.6.14**

This version adds an export activity panel below the progress bar, showing recent steps such as history discovery, active/archived/project/shared scans, and conversation processing. The activity panel restores when the popup is reopened. History discovery also reports the number found and elapsed time, while full-history JEX export estimates the remaining time. It includes full-history JEX ZIP export, resumable popup status, conversation-named documents, downloadable diagnostics, self-contained Markdown, ZIP64, multi-browser builds, incremental export, indexing, and checksums.
